...
import { Sparkle, ShoppingBag, ArrowLeft, CheckCircle } from "@phosphor-icons/react";
...
function CheckoutPageContent() {
  ...
  const handlePaymentSuccess = async (result: { orderId: string; paypalOrderId: string; total: number }) => {
    ...
    return (
      <div className="min-h-screen bg-[#fbf9f6] flex items-center justify-center">
        <div className="flex flex-col items-center gap-3">
          <svg className="animate-spin h-8 w-8 text-[#7C0A12]" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24">
      ...
      <div className="min-h-screen bg-[#fbf9f6] flex items-center justify-center p-6">
        <Card className="w-full max-w-md bg-white border border-[#78716C]/15 shadow-xl rounded-none">
          <CardHeader className="text-center pb-4">
            ...
            <CardTitle className="font-heading text-2xl text-[#1C1917]">Inicio de Sesión Requerido</CardTitle>
          ...
          <CardContent className="flex flex-col gap-4 text-center">
            <p className="text-xs text-[#625d5b] leading-relaxed">
              Por favor, regresa a la página de colecciones para iniciar sesión o registrarte y proceder con tu compra.
            </p>
            <Button
        ...
        <div className="w-full max-w-2xl bg-white border border-[#78716C]/15 shadow-2xl p-8 md:p-12 flex flex-col items-center text-center">
          ...
          <div className="w-full bg-[#fbf9f6] border border-[#78716C]/10 p-6 text-left flex flex-col gap-4 mb-8">
            <div className="flex justify-between items-center text-xs border-b border-[#78716C]/10 pb-3">
              <span className="text-[#78716C] uppercase font-semibold">Código de Pedido</span>
              <span className="font-mono text-[#1C1917] font-semibold">{completedOrder.orderId}</span>
            </div>
            <div className="flex justify-between items-center text-xs border-b border-[#78716C]/10 pb-3">
              <span className="text-[#78716C] uppercase font-semibold">PayPal Order ID</span>
              <span className="font-mono text-[#1C1917]">{completedOrder.paypalOrderId}</span>
            </div>
            <div className="flex justify-between items-center text-xs border-b border-[#78716C]/10 pb-3">
              <span className="text-[#78716C] uppercase font-semibold">Monto Pagado</span>
              <span className="text-[#7C0A12] font-bold text-sm">Bs. {completedOrder.total.toFixed(2)}</span>
            </div>
            
            <div className="flex flex-col gap-2 pt-1">
              <span className="text-[10px] text-[#78716C] uppercase tracking-wider font-bold">Detalle de Adquisición</span>
              {purchasedItems.map((item) => (
                <div key={item.product.id} className="flex justify-between text-xs py-1 text-[#625d5b]">
                  <span>{item.product.name} <span className="text-[#78716C]/60 font-semibold">x{item.quantity}</span></span>
          ...
          <p className="text-xs text-[#625d5b] leading-relaxed max-w-md mb-8">
            Tu transacción de simulación se completó de forma correcta y sin dinero real. Hemos registrado tu pedido en Supabase para el proyecto de la escuela.
          </p>
    ...
    return (
      <div className="min-h-screen bg-[#fbf9f6] flex items-center justify-center p-6">
        <div className="text-center flex flex-col items-center gap-4 max-w-sm">
          ...
          <h2 className="font-heading text-2xl text-[#1C1917]">Tu bolsa está vacía</h2>
          <p className="text-xs text-[#625d5b] leading-relaxed">
            No tienes piezas seleccionadas para el pago. Visita nuestro catálogo para añadir amuletos con intención a tu bolsa.
          </p>
          <Button
            onClick={() => window.location.href = "/colecciones"}
    ...
    <div className="min-h-screen bg-[#fbf9f6] pt-28 pb-16 px-6 md:px-12 max-w-7xl mx-auto selection:bg-[#7C0A12] selection:text-white">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-start">
        ...
        <div className="lg:col-span-7 flex flex-col gap-6">
          <h1 className="font-heading text-3xl md:text-4xl text-[#1C1917]">Finalizar Compra</h1>
                ...
                <div className="flex gap-4 items-center">
                  ...
                  <div className="flex flex-col gap-0.5">
                    ...
                    <p className="text-xs text-[#78716C]">
                      Piedra: <span className="font-semibold text-[#625d5b]">{item.product.stone}</span> &bull; Intención: <span className="font-semibold text-[#625d5b]">{item.product.intention}</span>
                    </p>
        ...
        <div className="lg:col-span-5 bg-white border border-[#78716C]/15 shadow-2xl p-6 md:p-8 flex flex-col gap-6">
          <h2 className="font-heading text-2xl text-[#1C1917]">Resumen del Pedido</h2>
          
          <div className="flex flex-col gap-3 text-xs border-b border-[#78716C]/10 pb-4 text-[#625d5b]">
            <div className="flex justify-between">
              <span>Subtotal</span>
              <span className="font-semibold">Bs. {cartSubtotal.toFixed(2)}</span>
            </div>
            <div className="flex justify-between">
              <span>Envío (Colección Permanente)</span>
              <span className="text-green-600 font-semibold uppercase">Gratuito</span>
            </div>
            <div className="flex justify-between">
          ...
          <div className="flex justify-between items-center py-2">
            <span className="font-heading text-lg text-[#1C1917] font-semibold">Total del Pedido</span>
            <span className="font-heading text-xl text-[#7C0A12] font-bold">Bs. {cartSubtotal.toFixed(2)}</span>
          </div>

          <div className="border border-yellow-200/80 bg-yellow-50/50 p-4 text-[10px] text-yellow-900/95 leading-relaxed">
            <p className="font-semibold mb-1 uppercase tracking-wider">Simulación Activa (Sandbox)</p>
            Esta pasarela opera en modo de pruebas. Utiliza credenciales de comprador de PayPal Developer Sandbox para pagar de forma simulada. No uses datos reales.
          </div>
